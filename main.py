...
async def main(client: Client, message: Message):
    ...
    if (message.text == "!start") and (message.from_user.id == (await client.get_me()).id):
        ...
        if Config.HEROKU_APP is None:
            await message.edit(
                text="Restarting Anime Feeder Userbot ...",
                parse_mode="Markdown",
                disable_web_page_preview=True
            )
        ...
        if len(Config.FORWARD_FROM_CHAT_ID) > 1:
            await message.edit(
                text="Sorry Onii Chaan,\nThere Is An Error In Our DBS",
                disable_web_page_preview=True
            )
            return
        await message.edit(
            text=f"Trying to Get All Anime Feeds from DBS and Forwarding to {' '.join(str(Config.FORWARD_TO_CHAT_ID))} ...",
            parse_mode="Markdown", disable_web_page_preview=True)
